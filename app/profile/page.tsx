  ...
  return (
    <ProductShell>
      <div className="flex flex-col gap-6 max-w-4xl mx-auto">
        <div>
          <Typography variant="h2" className="font-bold gold-gradient-text">My Profile Registry</Typography>
          <Typography variant="muted" className="text-xs">
            Manage your hyperlocal credentials, Aadhaar KYC verification, and trust score ledgers.
          </Typography>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
          ...
          <Card className="glass-card p-6 flex flex-col items-center gap-4 text-center md:col-span-1">
            ...
            <div>
              ...
              <Typography variant="muted" className="text-xs capitalize">{user.role} Member</Typography>
            </div>
            ...
            <div className="flex flex-col gap-1 w-full text-xs text-muted-foreground border-t border-border/20 pt-4 mt-2">
              ...
              <div className="flex justify-between">
                <span>Rating:</span>
                <span className="text-amber-500 font-bold">4.9 ★ (12 jobs)</span>
              </div>
              <div className="flex justify-between">
                <span>Location:</span>
                <span className="text-foreground">Guntur, Andhra Pradesh</span>
              </div>
            </div>
          </Card>
          ...
          <div className="md:col-span-2 flex flex-col gap-6">
            ...
            <Card className="glass-card p-6 flex flex-col gap-4">
              ...
              <div className="p-4 bg-muted/30 border border-border rounded-xl flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                <div>
                  <span className="text-xs font-semibold block">Aadhaar Validation Registry</span>
              ...
              {user.kycStatus === "unverified" && (
                <div className="flex flex-col gap-2 mt-2">
                  <label className="text-xs font-semibold text-muted-foreground">Select Identity PDF / Scan Image</label>
                  <div className="border border-dashed border-border rounded-xl p-6 text-center cursor-pointer transition-all hover:border-primary/40 flex flex-col items-center justify-center gap-2">
                    ...
                    <label htmlFor="onboard-kyc-upload-profile" className="text-[10px] bg-secondary border border-border px-2.5 py-1.5 rounded-lg text-foreground font-semibold cursor-pointer mt-1">Select File</label>
                  </div>
                </div>
              )}
            </Card>
            ...
            <Card className="glass-card p-6 flex flex-col gap-4">
              <Typography variant="h3" className="font-bold flex items-center gap-2">
                ...
                Trust & Safety Score Ledger
              </Typography>
              <p className="text-xs text-muted-foreground">
                Your trust score is calculated based on completed work SLAs, response times, escrow releases, and peer ratings on the JobNest decentralised ledger.
              </p>
              ...
              <div className="grid grid-cols-3 gap-4 border border-border rounded-xl p-4 text-center bg-muted/20">
                <div>
                  ...
                  <span className="block text-[10px] text-muted-foreground uppercase mt-1">SLA Accuracy</span>
                </div>
                <div>
                  <span className="text-xl font-bold text-emerald-400">100%</span>
                  <span className="block text-[10px] text-muted-foreground uppercase mt-1">Response Rate</span>
                </div>
                <div>
                  <span className="text-xl font-bold text-foreground">0</span>
                  <span className="block text-[10px] text-muted-foreground uppercase mt-1">Disputes</span>
                </div>
              </div>
            </Card>
