    ...
    <div className="min-h-screen bg-linear-to-b from-[#05050c] via-[#0b0c16] to-[#05050c] text-foreground flex flex-col justify-between items-center py-6 px-4 md:px-8 font-sans overflow-hidden">
      ...
      <div className="w-full max-w-xl flex justify-between items-center mb-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">{i18nT("app.workerPortal")}</span>
        </div>
          ...
          <motion.div
            ...
            <Card className="glass-panel border-border shadow-luxury backdrop-blur-md overflow-hidden flex flex-col min-h-120">
                <CardHeader className="pb-3 text-center">
                  ...
                  <CardTitle className="text-2xl font-bold tracking-tight text-foreground">{i18nT("app.buildYourHyperlocalFuture")}

                </CardTitle>
                  <CardDescription className="text-xs">{i18nT("app.connectInstantlyWithCustomersInYourNeighborhood")}
                ...
                <CardContent className="flex-1 flex flex-col justify-between gap-6 pt-2">
                  <div className="flex flex-col gap-4 text-xs text-muted-foreground leading-relaxed">
                    ...
                    <p className="bg-black/25 border border-border/30 rounded-xl p-3.5 flex items-center gap-3">
                      ...
                      <span>{i18nT("app.instantUpiPayoutsEscrowGuaranteesAndVerificationBadges")}</span>
                    </p>
                  </div>
                  ...
                  <div className="flex flex-col gap-2.5">
                    ...
                    className="w-full flex items-center justify-center gap-2 py-5 cursor-pointer">
                      ...
                      <span>{i18nT("app.createAccount")}</span>
                      <ArrowRight className="w-4 h-4" />
                    </Button>
                    <Button
                    variant="ghost"
                    onClick={() => {setAuthMode("signin");setStep(2);}}
                    className="w-full border border-border/40 cursor-pointer">{i18nT("app.signInToMyProfile")}
            ...
            <Card className="glass-panel border-border shadow-luxury backdrop-blur-md flex flex-col min-h-120">
                <CardHeader>
                  <CardTitle className="text-xl font-bold tracking-tight text-foreground flex items-center gap-2">
                    <Lock className="w-5 h-5 text-amber-500" />{i18nT("app.welcomeBack")}

                </CardTitle>
                  <CardDescription className="text-xs">{i18nT("app.signInToResumeFindingHyperlocalGigs")}
                ...
                <CardContent className="flex-1 flex flex-col justify-between gap-6">
                  <form onSubmit={handleAuthSubmit} className="flex flex-col gap-4">
                    ...
                    <div className="relative">
                      ...
                      placeholder={i18nT("app.enterPassword")}
                      required
                      value={password}
                    ...
                    <span className="text-[10px] text-amber-500 font-bold self-end cursor-pointer hover:underline">{i18nT("app.forgotPassword")}

                  </span>
                    <Button type="submit" variant="primary" className="w-full mt-2 cursor-pointer" isLoading={loading}>{i18nT("app.signIn")}
                  ...
                  <div className="flex flex-col gap-3">
                    <div className="flex items-center gap-2 text-[10px] text-muted-foreground uppercase tracking-widest justify-center">
                      ...
                      <span>{i18nT("app.orContinueWith")}</span>
                      <div className="h-px bg-border/40 flex-1" />
                    </div>
                    <Button
                    variant="ghost"
                    ...
                    className="w-full border border-border/40 text-xs flex items-center justify-center gap-2 cursor-pointer">
                      ...
                      <span>{i18nT("app.sandboxGoogleLoginBypass")}</span>
                    </Button>
                    <button onClick={prevStep} className="flex items-center justify-center gap-1.5 text-xs text-muted-foreground hover:text-foreground mt-2 cursor-pointer">
                      ...
                      <span>{i18nT("app.backToWelcome")}</span>
                    </button>
                  </div>
                </CardContent>
              </Card>
            ...
            <Card className="glass-panel border-border shadow-luxury backdrop-blur-md flex flex-col min-h-120">
                <CardHeader>
                  <CardTitle className="text-xl font-bold tracking-tight text-foreground flex items-center gap-2">
                    <User className="w-5 h-5 text-amber-500" />{i18nT("app.createWorkerAccount")}

                </CardTitle>
                  <CardDescription className="text-xs">{i18nT("app.getVerifiedAndUnlockDirectPayoutsInstantly")}
                ...
                <CardContent className="flex-1 flex flex-col justify-between gap-6">
                  <form onSubmit={handleAuthSubmit} className="flex flex-col gap-4">
                    ...
                    <div className="relative">
                      ...
                      label={i18nT("Password")}
                      type={showPassword ? "text" : "password"}
                      placeholder={i18nT("app.atLeast6Characters")}
                      required
                      value={password}
                      onChange={(e) => setPassword(e.target.value)} />
                  ...
                  <div className="flex items-center gap-1.5 mt-0.5">
                    ...
                    <Button type="submit" variant="primary" className="w-full mt-2 cursor-pointer" isLoading={loading}>{i18nT("app.createAccount")}
            ...
            <Card className="glass-panel border-border shadow-luxury backdrop-blur-md flex flex-col min-h-120">
                <CardHeader>
                  <CardTitle className="text-xl font-bold tracking-tight text-foreground flex items-center gap-2">
                    <MapPin className="w-5 h-5 text-amber-500 animate-bounce" />{i18nT("app.enableHyperlocalMatching")}

                </CardTitle>
                  <CardDescription className="text-xs">{i18nT("app.jobnestMatchesYouWithCustomersInYourVicinity")}
                ...
                <CardContent className="flex-1 flex flex-col justify-between gap-6">
                  <div className="bg-black/35 rounded-xl border border-border/40 p-4 text-xs text-muted-foreground flex flex-col gap-3 leading-relaxed">
                    <p className="font-semibold text-foreground">{i18nT("app.whyDoWeNeedLocationAccess")}</p>
                    <p>{i18nT("app.customersCanCalculateYourRealtimeEtaAndTravel")}</p>
                    <p>{i18nT("app.receiveLivePushNotificationsWhenACustomerPosts")}</p>
                    <p className="text-[10px] text-amber-500 font-medium">{i18nT("app.yourCoordinatesAreEncryptedAndOnlySharedWith")}</p>
                  </div>

                  {/* Manual search backup */}
                  <div className="flex flex-col gap-1.5">
                    <label className="text-[10px] uppercase font-bold text-muted-foreground" htmlFor="city-search">{i18nT("app.orSearchYourCityManually")}

                  </label>
                    <div className="flex gap-2">
                      <Input
                      id="city-search"
                      placeholder={i18nT("app.egGunturBangalore")}
                      value={citySearch}
                      ...
                      className="border border-border/40 text-xs px-3 cursor-pointer">{i18nT("app.setCity")}
                  ...
                  <div className="flex flex-col gap-2.5">
                    ...
                    <div className="flex justify-between items-center text-xs mt-1">
                      ...
                      <button onClick={nextStep} className="text-amber-500 font-semibold hover:underline cursor-pointer">{i18nT("app.skipSetLater")}
            ...
            <Card className="glass-panel border-border shadow-luxury backdrop-blur-md flex flex-col min-h-120">
                <CardHeader>
                  <CardTitle className="text-xl font-bold tracking-tight text-foreground flex items-center gap-2">
                    <Sparkles className="w-5 h-5 text-amber-500" />{i18nT("app.completeWorkerProfile")}

                </CardTitle>
                  <CardDescription className="text-xs">{i18nT("app.thisInfoBuildsCustomerTrust")}
                ...
                <CardContent className="flex-1 flex flex-col justify-between gap-5">
                  <div className="flex flex-col gap-4 overflow-y-auto max-h-72.5 pr-1">
                    ...
                    <div className="flex items-center gap-4 border-b border-border/20 pb-3">
                      ...
                      <div className="flex flex-col gap-1.5">
                        <label htmlFor="profile-avatar" className="text-xs bg-amber-600 hover:bg-amber-700 text-background px-3 py-1.5 rounded-lg font-bold cursor-pointer inline-block text-center">{i18nT("app.uploadPhoto")}

                      </label>
                        <input type="file" id="profile-avatar" accept="image/*" className="hidden" onChange={handlePhotoUpload} />
                        <span className="text-[9px] text-muted-foreground">{i18nT("app.jpgpngMaxSize2mb")}</span>
                      </div>
                    </div>

                    <Input
                    label={i18nT("app.contactPhoneNumber")}
                    type="tel"
                    placeholder={i18nT("app.eg919876543210")}
                    required
                    value={phoneNumber}
                    ...
                    <div className="flex flex-col gap-1.5">
                      <label className="text-sm font-semibold text-muted-foreground">{i18nT("app.experienceLevel")}</label>
                      <select
                      value={experience}
                      ...
                      className="bg-muted text-foreground text-xs font-semibold px-3 py-2 rounded-xl border border-border outline-none cursor-pointer">
                        ...
                        <option value="1">{i18nT("app.1YearOrLess")}</option>
                        <option value="2">{i18nT("app.23Years")}</option>
                        <option value="5">{i18nT("app.5YearsExpert")}</option>
                        <option value="10">{i18nT("app.10YearsMaster")}</option>
                      </select>
                    </div>

                    <div className="flex flex-col gap-1.5">
                      <label className="text-sm font-semibold text-muted-foreground">{i18nT("app.aboutProfessionalBio")}</label>
                      <textarea
                      value={aboutBio}
                      onChange={(e) => setAboutBio(e.target.value)}
                      rows={3}
                      placeholder={i18nT("app.stateYourToolsAreasOfExpertiseAndService")}
            ...
            <Card className="glass-panel border-border shadow-luxury backdrop-blur-md flex flex-col min-h-120">
                <CardHeader>
                  <CardTitle className="text-xl font-bold tracking-tight text-foreground flex items-center gap-2">
                    <Award className="w-5 h-5 text-amber-500" />{i18nT("app.selectYourSkills")}

                </CardTitle>
                  <CardDescription className="text-xs">{i18nT("app.chooseCategoriesAndGetDynamicAiRecommendations")}
                ...
                <CardContent className="flex-1 flex flex-col justify-between gap-5">
                  <div className="flex flex-col gap-4 overflow-y-auto max-h-75 pr-1">
                    ...
                    <form onSubmit={handleAddCustomSkill} className="flex gap-2">
                      <Input
                      placeholder={i18nT("app.searchOrAddSkill")}
                      value={skillQuery}
                  ...
                  <div className="flex flex-col gap-1.5">
                        <label className="text-[10px] uppercase font-bold text-muted-foreground">{i18nT("app.mySelectedSkills")}</label>
                    ...
                    <div className="flex flex-col gap-1.5">
                      <label className="text-[10px] uppercase font-bold text-muted-foreground">{i18nT("app.popularHandymanSkills")}</label>
                      <div className="flex flex-wrap gap-1.5">
                    ...
                    <div className="bg-amber-950/20 border border-amber-500/20 rounded-xl p-3.5 flex flex-col gap-2">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-[10px] uppercase font-bold text-amber-400">{i18nT("app.aiDemandSuggestionsGuntur")}</span>
                      </div>
                      <div className="flex flex-wrap gap-1.5">
                        {aiSuggestions.map((s) =>
                      <button
            ...
            <Card className="glass-panel border-border shadow-luxury backdrop-blur-md flex flex-col min-h-120">
                <CardHeader>
                  <CardTitle className="text-xl font-bold tracking-tight text-foreground flex items-center gap-2">
                    <Clock className="w-5 h-5 text-amber-500" />{i18nT("app.configureWorkPreferences")}

                </CardTitle>
                  <CardDescription className="text-xs">{i18nT("app.definePayoutRangesAndHyperlocalOperatingBoundaries")}
                  ...
                  <div className="flex flex-col gap-4 overflow-y-auto max-h-75 pr-1">
                    ...
                    <div className="flex flex-col gap-2 border border-border/20 rounded-xl p-3.5 bg-black/10">
                      <div className="flex justify-between text-xs font-semibold">
                        <span className="text-muted-foreground">{i18nT("app.hyperlocalTravelDistance")}</span>
                      ...
                      <span className="text-[10px] text-muted-foreground">{i18nT("app.receiveClientRequestsUpTo")}{workRadius}{i18nT("app.kilometersAway")}</span>
                    </div>

                    {/* Expected Payout Slider */}
                    <div className="flex flex-col gap-2 border border-border/20 rounded-xl p-3.5 bg-black/10">
                      <div className="flex justify-between text-xs font-semibold">
                        <span className="text-muted-foreground">{i18nT("app.expectedDailyRate")}</span>
                        <span className="text-amber-500 font-bold">₹{earnings}{i18nT("app.day")}</span>
                      </div>
                      <input
                      type="range"
                      min="300"
                      ...
                      <span className="text-[10px] text-muted-foreground">{i18nT("app.helpsSuggestPricingParametersForBookedJobsAutomatically")}</span>
                    </div>

                    {/* Availability Select */}
                    <div className="flex flex-col gap-1.5">
                      <label className="text-xs font-semibold text-muted-foreground">{i18nT("app.availabilitySchedule")}</label>
            ...
            <Card className="glass-panel border-border shadow-luxury backdrop-blur-md flex flex-col min-h-120">
                <CardHeader className="text-center">
                  ...
                  <CardTitle className="text-xl font-bold tracking-tight text-foreground">{i18nT("app.onboardingCompleted")}

                </CardTitle>
                  <CardDescription className="text-xs">{i18nT("app.yourProfileRegistryIsNowLiveInGuntur")}
                ...
                <CardContent className="flex-1 flex flex-col justify-between gap-5">
                  <div className="bg-black/35 border border-border/30 rounded-xl p-4 flex flex-col gap-3 text-xs leading-relaxed">
                    <div className="flex justify-between items-center border-b border-border/10 pb-2">
                      <span className="text-muted-foreground">{i18nT("app.fullName")}</span>
                      <span className="font-bold text-foreground">{fullName}</span>
                    </div>
                    <div className="flex justify-between items-center border-b border-border/10 pb-2">
                      <span className="text-muted-foreground">{i18nT("app.hyperlocalDistance")}</span>
                      <span className="font-bold text-amber-500">{workRadius}{i18nT("Km")}</span>
                    </div>
                    <div className="flex justify-between items-center border-b border-border/10 pb-2">
                      <span className="text-muted-foreground">{i18nT("app.selectedSkills")}</span>
                      <span className="font-bold text-foreground truncate max-w-50">{selectedSkills.join(", ")}</span>
                    </div>
                    <div className="flex justify-between items-center pb-1">
                      <span className="text-muted-foreground">{i18nT("app.profileStatus")}</span>
                      <Badge variant="success" className="bg-emerald-950/60 border border-emerald-500/30 text-emerald-400">{i18nT("app.100Complete")}</Badge>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col gap-2.5">
                    ...
                    isLoading={loading}>
                      ...
                      <span>{i18nT("app.startFindingOpportunities")}</span>
                      <ArrowRight className="w-4 h-4 animate-pulse" />
                    </Button>
                    <button onClick={prevStep} className="text-xs text-muted-foreground hover:text-foreground self-center cursor-pointer">{i18nT("app.reviewPreferences")}
      ...
      <div className="w-full max-w-xl text-center mt-4">
        <Typography variant="muted" className="text-[10px] leading-relaxed flex items-center justify-center gap-1.5 text-muted-foreground/60">
          ...
          <span>{i18nT("app.jobnestVerifiedWorkerTrustNetworkAadhaarKycVerified")}</span>
        </Typography>
      </div>
    </div>);
